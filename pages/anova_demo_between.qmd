---
title: "2 x 2 Between Design ANOVA"
author: "Boris Mayer"
format: 
  html:
    resources:
...
akademisch <- read_csv("akademisch.csv") |> 
    mutate(bildung_vater = factor(bildung_vater, levels = (c("niedrig", "hoch"))),
           bildung_mutter = factor(bildung_mutter, levels = (c("niedrig", "hoch"))))
...
## B) 2 x 2 ANOVA mit afex und emmeans
...
Leider erhalten wir keinen Gesamteffekt. Diesen bekommen wir als letzte Zeile hier:
...
Standardmässig werden (wie z.B. auch in jamovi oder SPSS) die partiellen Effektgrössen berechnet. Diese werden bei experimentell manipulierten Faktoren empfohlen. Bei beobachteten Faktoren wie in diesem Beispiel sollte man auf nicht-partielle Effektgrössen zurückgreifen: 

```{webr-r}
anovaBETWEEN_obs <- aov_4(akad_swk ~ bildung_mutter * bildung_vater + (1 | id), data = akademisch, observed = c("bildung_mutter", "bildung_vater"))
...
$\hat\eta^2_{pBM} = \frac{QS_{BM}}{QS_{BM} + QS_{Res}} = \frac{9.122}{9.122 + 143.216} = `r 9.122/(9.122 + 143.216)`$

$\hat\eta^2_{pBM} = \frac{QS_{BM}}{QS_{TOT}} = \frac{9.122}{9.122 + 0.027 + 1.487 + 143.216} = `r 9.122/(9.122 + 0.027 + 1.487 + 143.216)`$

Wenn die Faktoren als `observed` definiert sind, kann man beide (partielle und nicht-partielle) Effektgrössen auch direkt anfordern:

```{webr-r}
anovaBETWEEN_obs2 <- aov_4(akad_swk ~ bildung_mutter * bildung_vater + (1 | id), data = akademisch, observed = c("bildung_mutter", "bildung_vater"), anova_table = list(es = c("ges", "pes")))
...
```{webr-r}
anovaBETWEEN_eff <- aov_4(akad_swk ~ bildung_mutter * bildung_vater + (1 | id), data = akademisch, anova_table = list(es = c("ges", "pes")))
...
```{webr-r}
p1 <- afex_plot(object = anovaBETWEEN,
               x = "bildung_mutter",
               trace = "bildung_vater",
               mapping = c("color", "shape"), 
               # Faktorstufen sollen unterschiedliche Farben und Formen erhalten
               dodge = 0.2, 
               # Verschiebung der Faktorstufen reduzieren (default: 0.5)
               error_ci = FALSE, 
               # Standardfehler statt Konfidenzintervalle als Fehlermass
               legend_title = "Bildung Vater",
               # Legendentitel ist nur hier anpassbar! (nicht mit ggplot())
               data_plot = FALSE) 
...
Jetzt können wir `pairs()` mit `simple` für denjenigen Faktor verwenden, für den die bedingten Haupteffekte berechnet werden sollen. Ausserdem gibt es eine Funktion `eff_size()`, mit der Cohens' d für die Vergleiche berechnet werden kann.
...
Vergleiche aller vier Bedingungen untereinander (inkl. "diagonale Bedingungen", d.h. Bedingungen, die sich auf beiden Faktoren unterscheiden), jetzt einmal ohne und einmal mit Alpha-Korrektur:
